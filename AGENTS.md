...
- `AGENTS.md`・`CLAUDE.md`・`.claude/skills/` の共有ミラー・`.agents/skills/`・フック設定は生成物である。ソース(`harness/AGENTS.md`・`PROJECT.md`・`harness/skills/`、フック設定は CLI の `src/`)を編集し、`cargo run -- sync --source .` で再生成する。
