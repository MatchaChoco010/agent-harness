...
| やること | コマンド |
|---|---|
| Issue/PR の作成・閲覧・`gh api` など汎用の GitHub 操作 | `node harness/scripts/gh/gh.mjs <gh の引数...>` |
| コミットを bot 名義(Verified)で作る(`git commit` の代わり) | `git add ...` してから `node harness/scripts/gh/commit.mjs "メッセージ"` |
| コンフリクト解消済みマージを bot 名義の 2 親マージコミットで確定(`git merge --continue` の代わり) | 解消して `git add ...` してから `node harness/scripts/gh/merge-commit.mjs "メッセージ"` |
| PR レビューコメントの取得(未解決スレッド + 会話) | `node harness/scripts/gh/pr-comments.mjs <PR番号> [--all] [--json]` |
| PR レビューコメントへのスレッド内返信 | `node harness/scripts/gh/pr-reply.mjs <PR番号> <コメントID> <本文>` |
| 認証情報の疎通確認(トークンは出力しない) | `node harness/scripts/gh/app-token.mjs --check` |
