...
//   BOT_GH_APP_ID          … GitHub App の App ID(数字)
//   BOT_GH_INSTALLATION_ID … リポジトリにインストールした Installation ID(数字)
//   BOT_GH_APP_KEY         … 秘密鍵 .pem の絶対パス(リポジトリ外)
//
// 使い方(モジュール):
...
function config() {
  ...
  if (!keyPath) missing.push('BOT_GH_APP_KEY')
  if (missing.length) {
    throw new Error(
...
function makeJwt({ appId, keyPath }) {
  ...
  try {
    ...
    throw new Error(`秘密鍵を読めません (BOT_GH_APP_KEY=${keyPath}): ${e.message}`)
  }
...
function cachePath({ appId, installationId }) {
  return join(tmpdir(), `bot-gh-app-token-${appId}-${installationId}.json`)
}
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (mode === '--check') {
    ...
    console.error('このスクリプトはモジュールとして使う。診断は: node harness/scripts/gh/app-token.mjs --check')
    console.error('gh を bot 名義で実行するには: node harness/scripts/gh/gh.mjs <gh の引数...>')
    process.exit(2)
  }
}
