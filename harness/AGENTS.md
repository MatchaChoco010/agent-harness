...
**ゲーティング PR のマージはユーザーが行う。エージェントは自分の判断で `develop` / `main` にマージしない**(ユーザーが明示的に指示したときの代行と、唯一の例外であるレビュー前 Design Doc の develop 集約 landing を含め、[harness/docs/git-and-pr.md](harness/docs/git-and-pr.md) が正)。
