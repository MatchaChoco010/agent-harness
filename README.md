...
2. design doc を依頼した場合、エージェントが `docs/design/NNNN_*.md` に書き上げて `develop` に載せ、続けてレビュー PR を立てる。doc の本文は `develop` 上にあるので、ブランチを切り替えずに読める。
3. レビュー PR の差分はステータスの 1 行だけだが、GitHub の Files changed から doc の全文にコメントできる。
